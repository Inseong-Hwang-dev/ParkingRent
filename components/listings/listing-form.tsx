...
function validateStep(step: number, form: FormState): Errors {
  ...
  if (step === 3) {
    const anyEnabled =
      form.daily_enabled || form.fortnightly_enabled || form.monthly_enabled
    if (!anyEnabled) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { listingId } = await createListing({
        ...
        price_fortnightly: form.fortnightly_enabled
          ? parseFloat(form.price_fortnightly)
          : null,
